...
/* Parfois, quand le personnage recule vers un mur, ou quand il tourne sur lui-même à proximité d'un mur,
   la caméra se retrouve à l'intérieur du mur. Et donc on ne voit que le cube qui contient la caméra.
   Il faut donc faire avancer la caméra en direction du personnage jusqu'à ce que le terrain
   ne vienne plus s'interposer. */
void avancer_cam_pour_mur(struct camera*c, struct terrain*t){
	
	float d_mini=0.01;
	c->distance_compensation_mur=0;
	while(terrain_contient_point(t, &(c->empty.pos))){
		deplacer_cam_local(c, d_mini, 0, 0);
		normer_vecteur(&(c->empty.cible), &(c->empty.cible));
		mult_vect_scal(&(c->empty.cible), distance_entre_emptys(&(c->empty), &(c->le_perso->empty)), &(c->empty.cible));
		c->distance_compensation_mur+=d_mini;
	}
}

/* On a avancé la caméra pour la faire sortir du mur;
   maintenant on veut la replacer au point initial.
   On a mémorisé la distance d'avancement dans le champ 'distance_compensation_mur' de la caméra. */
void reculer_cam_pour_mur(struct camera*c){
	deplacer_cam_local(c, -c->distance_compensation_mur, 0, 0);
	normer_vecteur(&(c->empty.cible), &(c->empty.cible));
	mult_vect_scal(&(c->empty.cible), distance_entre_emptys(&(c->empty), &(c->le_perso->empty)), &(c->empty.cible));
}
